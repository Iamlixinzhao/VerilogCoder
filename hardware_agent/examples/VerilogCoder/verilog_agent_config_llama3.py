...
def get_plan_graph_retrieval_agent_config(config_list: Dict[str, Any], opensource_model=True):
    if opensource_model:
...
def get_verilog_completion_agent_config(config_list: Dict[str, Any], opensource_model=True):
    if opensource_model:
        assistance_agent = "OS_AssistantAgent"
    else:
        assistance_agent = "AssistantAgent"
...
def get_verilog_waveform_debug_agent_config(config_list: Dict[str, Any],opensource_model=True):
    if opensource_model:
        assistance_agent = "OS_AssistantAgent"
    else:
        assistance_agent = "AssistantAgent"
    ...
    code_executor = LocalCommandLineCodeExecutor(work_dir="coding")
    # For more than 3 agents
    solving_group_chat_configs = {'group_chat': {'speaker_selection_method': "auto",
                                                 'messages': [],
                                                 'max_round': 40},
                                  'chat_manager': {
                                      'is_termination_msg': lambda x: x.get("content", "") and x.get("content",
...
def get_verilog_waveform_debug_agent_singleLLM_config(config_list: Dict[str, Any],opensource_model=True):
    if opensource_model:
        assistance_agent = "OS_AssistantAgent"
    else:
        assistance_agent = "AssistantAgent"
...
def get_verilog_debug_agent_config(config_list: Dict[str, Any], opensource_model=True):
    if opensource_model:
        assistance_agent = "OS_AssistantAgent"
    else:
        assistance_agent = "AssistantAgent"
    ...
    solving_agent_configs = [
        {'type': 'UserProxyAgent',
         'tools': ['verilog_simulation_tool', 'recall_spec_and_generated_verilog_code_tool'],
         'base_agent_config': {'name': 'user',
                               'description': 'User proxy who ask questions and execute the tools with the provided input from Assistant.',
                               'human_input_mode': "NEVER",
                               'is_termination_msg': lambda x: x.get("content", "") and x.get("content",
        ...
        {'type': assistance_agent,
         'tools': ['recall_spec_and_generated_verilog_code_tool'],
         'transform_message': {'method': ['HistoryLimit'],
                               'args': [{'max_messages': 3}]},
         # 'transform_message': {'method': ['LLMSummary'],
         #                       'args': [{'llm_config': {"config_list": config_list, "cache_seed": None},
         #                                 'max_token': 800}]},
         'base_agent_config': {'name': 'debug_engineer',
                               'llm_config': {"config_list": config_list, "cache_seed": None, "temperature": 0.2,
                                              "top_p": 1},
                               'description': "Debug engineer to do 1. use the tool to debug and detect root cause, 2. provide a bug fix plan.",
                               'is_termination_msg': lambda x: x.get("content", "") and x.get("content",
                                                                                              "").rstrip().endswith(
                                   "TERMINATE"),
                               'max_consecutive_auto_reply': 40,
                               # the default system message of the AssistantAgent is overwritten here
                               'system_message': "You are a verilog RTL debugging engineer. You use the provided tools and suggest the verilog debugging "
                                                 " plan for verilog engineer to finetune the verilog code. Do not suggest concrete "
                                                 "code. For any action beyond writing code or reasoning, convert it to a step that "
                                                 "can be implemented by finetuning code. If the execution is wrong, analyze the error "
                                                 "and suggest a fix.",
                               }
         },
    ]
